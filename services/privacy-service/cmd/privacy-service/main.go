// Comando privacy-service — direitos do titular LGPD (exportação/exclusão).
package main
...
import (
	...
	"opensight/services/privacy-service/internal/api"
	"opensight/services/privacy-service/internal/repository"
)

func main() {
	log.SetFlags(log.LstdFlags | log.LUTC)
	cfg := httpkit.LoadConfig("8092")
	...
	var repo repository.PrivacyRepository
	if cfg.DatabaseURL != "" {
	...
	go func() {
		log.Printf("privacy-service ouvindo em :%s", cfg.Port)
