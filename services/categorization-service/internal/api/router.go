...
func NewRouter(repo repository.CategoryRepository, cfg httpkit.Config, authPub *rsa.PublicKey) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/categories/overview", func(w http.ResponseWriter, r *http.Request) {
		ov, err := repo.Overview(r.Context(), httpkit.UserID(r.Context()))
		if err != nil {
			httpkit.Error(w, http.StatusInternalServerError, "internal", "falha ao carregar categorias")
			return
		}
