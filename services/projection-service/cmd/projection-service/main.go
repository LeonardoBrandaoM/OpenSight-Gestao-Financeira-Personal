// Comando projection-service — projeções de patrimônio (cenários).
package main
...
import (
	...
	"opensight/services/projection-service/internal/api"
	"opensight/services/projection-service/internal/repository"
)

func main() {
	log.SetFlags(log.LstdFlags | log.LUTC)
	cfg := httpkit.LoadConfig("8086")
	...
	var repo repository.ProjectionRepository
	if cfg.DatabaseURL != "" {
	...
	go func() {
		log.Printf("projection-service ouvindo em :%s", cfg.Port)
