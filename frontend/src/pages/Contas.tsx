...
import { contas, type Conta } from '../data/mock';
import { DeltaChip, Panel } from '../components/ui';
...
// Índice original de cada conta (estável p/ rotas /contas/:id e cor consistente).
const comIndice = contas.map((c, i) => ({ ...c, idx: i }));
...
// "Dinheiro da pessoa" = soma dos saldos positivos (ativos).
const ativos = contas.reduce((s, c) => s + (c.saldo > 0 ? c.saldo : 0), 0);
const share = (saldo: number) => (ativos > 0 && saldo > 0 ? (saldo / ativos) * 100 : 0);

// Cartão de conta padrão (corrente/poupança/visão geral) com participação nos ativos.
function AccountCard({ c }: { c: ContaIdx }) {
  const negativo = c.saldo < 0;
  const pct = share(c.saldo);
  ...
  const stats = (() => {
    if (aba === 'corrente') {
      ...
      return [
        ...
        { label: '% dos ativos', value: `${share(soma).toFixed(1)}%` },
      ];
    }
    if (aba === 'credito') {
